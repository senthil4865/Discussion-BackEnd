...
let loginFunction = (req, res) => {
    ...
    let generateToken = (userDetails) => {
        console.log(userDetails,'generate token');
    ...
    let saveToken = (tokenDetails) => {
        console.log(tokenDetails,'inside save token');
                ...
                if (err) {
                    ...
                    newAuthToken.save((err, newTokenDetails) => {
                        if (err) {
                            ...
                            let apiResponse = response.generate(false, 'Login Successful', 200, responseBody)
                           res.send(apiResponse);
                        }
                    })
                } else {
            ...
            User.update({ 'email': req.body.email }, options).exec((err, result) => {
                if (err) {
                    ...
                    let sendEmailOptions = {
                        ...
                        subject: 'Reset Password for Discussion ',
            ...
            User.update({ 'userId': userDetails.userId }, options).exec((err, result) => {
                if (err) {
                    ...
                    let sendEmailOptions = {
                        ...
                        subject: 'Password Updated for Discussion ',
                        html: `<h4> Hi ${userDetails.firstName}</h4>
                        <p>
                            Password updated successfully.
                        </p>
                        <h3> Thanks for using Discussion </h3>
                                    `
                    }
